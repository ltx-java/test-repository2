...
<script type="text/html" id="tpl">
    ...
    <tr>
        ...
        <td>
            <a href="javascript:;" data-id="{{ item.id }}" data-opt="view"
               class="layui-btn layui-btn-normal layui-btn-mini">详情</a>
            <a href="javascript:;" data-id="{{ item.id }}" data-opt="notify" class="layui-btn layui-btn-mini">编辑</a>
...
<script>
    ...
    layui.use(['paging', 'form'], function () {
        ...
        paging.init({
            openWait: true,
            url: '/user_info/list?v=' + new Date().getTime(), //地址
            elem: '#content', //内容容器
            params: { //发送到服务端的参数
            },
            type: 'GET',
        ...
        function viewForm(id) {
            ...
            $.get('/user_info/view.html?id=' + id, null, function (form) {
        ...
        function editForm(id) {
            //本表单通过ajax加载 --以模板的形式，当然你也可以直接写在页面上读取
            $.get('/user_info/edit.html?id=' + id, null, function (form) {
                addBoxIndex = layer.open({
                    type: 1,
                    title: '保存用户',
                    content: form,
        ...
        function delRecord(id) {
            layer.confirm('真的删除ID为：'+id+"的用户吗?", function(index){
